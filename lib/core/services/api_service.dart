...
class ApiService {
  ...
  Future<Map<String, dynamic>?> login(String mobile, String password) async {
    ...
    final phone = normalized.length > 10 ? normalized.substring(normalized.length - 10) : normalized;
    final response = await http.post(
      Uri.parse('$_baseUrl/auth/login'),
      headers: {'Content-Type': 'application/json'},
      body: json.encode({'mobile': phone, 'password': password}),
    );
  ...
  // Verify OTP via API
  Future<Officer?> verifyOtp(String mobile, String otp) async {
    final normalized = mobile.replaceAll(RegExp(r'[^\d]'), '');
    final phone = normalized.length > 10 ? normalized.substring(normalized.length - 10) : normalized;
    final response = await http.post(
